...
pub async fn read_string_or_init(
    ...
    _read_string_or_init(path.as_ref(), contents.as_ref()).await
}

async fn _read_string_or_init(path: &Path, contents: &[u8]) -> io::Result<String> {
...
#[cfg(test)]
mod tests {
    use {super::read_string_or_init, std::io, tempfile::tempdir, tokio::fs};
    ...
    async fn returns_existing_content() {
        ...
        let out = read_string_or_init(&file, "default").await.unwrap();
        assert_eq!(out, "hi");

        // file content unchanged
        assert_eq!(fs::read_to_string(&file).await.unwrap(), "hi");
    }
    ...
    async fn creates_and_returns_default_when_missing() {
        ...
        let out = read_string_or_init(&file, "hello").await.unwrap();
        assert_eq!(out, "hello");
    ...
    async fn error_when_existing_file_not_utf8() {
        ...
        let err = read_string_or_init(&file, "default").await.unwrap_err();
    ...
    async fn error_when_default_content_not_utf8() {
        ...
        let err = read_string_or_init(&file, vec![0xFF, 0xFE, 0xFD]).await.unwrap_err();
        assert_eq!(err.kind(), io::ErrorKind::InvalidData);
    }
}
